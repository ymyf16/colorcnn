...
import functools
import copy
from nefesi2022.functions.similarity_index import get_row_of_similarity_index
...
class LayerData(object):
    ...
    def get_receptive_field(self,model,input_shape,main_input=None):
        ...
        base_model = getattr(model, "pytorchmodel", model)
        if self.decoder:
            n, K, S, P = decoder_rf(base_model, input_shape, self.layer_id, main_input=main_input)
        else:
            n, K, S, P = gradient_rf(base_model, input_shape, self.layer_id, main_input=main_input)
        self.neurons_data = n
        self.receptive_field_Kernel = K
        self.receptive_field_Stride = S
        self.receptive_field_Padding = P
