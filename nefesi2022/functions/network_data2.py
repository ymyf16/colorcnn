...
from nefesi2022.interface_DeepFramework.DeepFramework import data_batch_generator, deep_model

from nefesi2022.functions.neuron_data2 import NeuronData

from nefesi2022.functions.general_functions import get_key_of_index
from nefesi2022.functions.layer_data2 import LayerData
from nefesi2022.functions.neuron_feature2 import compute_nf, compute_nf_out
from nefesi2022.functions.image import ImageDataset
from nefesi2022.functions.read_activations import fill_all_layers_data_batch
...
class NetworkData(object):
    ...
    def layers_data(self, layer_data):
        ...
        if type(layer_data) is list:
            for layer_name in layer_data:
                ...
                base_model = getattr(self.model, 'pytorchmodel', self.model)
                layer_d.get_receptive_field(base_model, self.input_shape, main_input=0) #allow raw model to pass
    ...
    def eval_network(self, layer_names = None,
        ...
        for n_batches, imgs in enumerate(data_batch.iterator):
            ...
            # base_model = getattr(self.model, 'pytorchmodel', self.model)
            base = getattr(self.model, 'pytorchmodel', self.model)
            fill_all_layers_data_batch(actual_file_names, images, base, self.layers_data)
